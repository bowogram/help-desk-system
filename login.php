...
if(isset($_POST['buttonSignIn'])){
	if(isset($_POST['loginemail']) && isset($_POST['loginpassword'])){
		...
		$user =$_POST['user'];
		if($user=="1"){
			if ($objCus->isCustomer($email, $password))
			 {
				 $url = "index2.php";
				Login::loginFront($objCus->_id, $url);
			} else {
		...
		if($user=="2"){
			if ($objTech->isTechnician($email, $password))
			 {
				 $url = "technician/tech.php";
				Login::loginTechnician($objTech->_id, $url);
			} else {
				$objValid->add2Errors('login');
			}
		} 
